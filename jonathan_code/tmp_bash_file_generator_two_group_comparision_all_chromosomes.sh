...
echo "#!/bin/bash
" > $out_directory/tmp_bash_all_marks_"$sample_type1"_"$sample_type2"_"$p_value_threshold".sh
...
declare -a marks=( "H3K27ac" "H3K27me3" "H3K9me3" "H3K4me3" "H3K4me1" "H3K36me3" )

for c in "${chromosomes[@]}"; do for m in "${marks[@]}"; do
echo "bash /projects/blueprint_CLL_dart2/jsteif/scripts/run_opt_two_group_comparision.sh -t $sample_type1 -T $sample_type2 -p $p_value_threshold -c $c -m $m -i $input_tables_directory -g $groups_column -o $out_directory" >> $out_directory/tmp_bash_all_marks_"$sample_type1"_"$sample_type2"_"$p_value_threshold".sh ; done; done
