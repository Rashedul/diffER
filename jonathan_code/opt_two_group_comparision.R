...
spec = matrix(c(
  ...
  "groups_column", "g", 1, "character", "the name of the metadata grouping column e.g sample_type",
...
meta_df <- read.table("/projects/blueprint_CLL_dart2/jsteif/metadata/metadata_table_corrected_version2.csv", stringsAsFactors = FALSE, header = T, sep=",") %>% 
...
a <- group_peak_count_table_maker_chr(meta_df %>% filter(!!as.symbol(opt$groups_column) == opt$sample_type1), opt$chromosome, opt$mark)
b <- group_peak_count_table_maker_chr(meta_df %>% filter(!!as.symbol(opt$groups_column) == opt$sample_type2), opt$chromosome, opt$mark)
