...
bash /projects/blueprint_CLL_dart2/jsteif/scripts/run_opt_two_group_comparision.sh -t $sample_type1 -T $sample_type2 -c $chromosome -p $p_value_threshold -m $mark -i $input_tables_directory -g $groups_column -o $out_directory" > $out_directory/sbatch_"$mark"_"$sample_type1"_"$sample_type2"_"$chromosome"_"$p_value_threshold".sh
