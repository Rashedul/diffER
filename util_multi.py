...
def make_windows_build(genome_build, window_size, temp_dir):
    ...
    # Save the windows to a new BED file in the provided temp directory
    filepath = os.path.join(temp_dir, output_bed)
    windows.saveas(filepath)
    print(f"Windows saved to: {filepath}")

def make_windows_file(genome_file, window_size, temp_dir):
    print("Generating windows from genome file...")
    # Create a BED object from the genome file
    a = pybedtools.BedTool(genome_file)

    # Create windows of specified size
    windows = a.window_maker(b=a, w=window_size)
    output_bed = "windows.bed"
...
def enriched_regions(fisher_p_value, merge_intervals, differ_output_filename):
    ...
    df = pd.read_csv('merged_group_A_B_fisher.bed', sep='\t', header=None)
